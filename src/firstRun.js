...
export async function runFirstRunSetup() {
  ...
  while (step <= 4) {
    if (step === 1) {
      ...
      saveConfig(CONFIG_NAME, { ...loadConfig(CONFIG_NAME, {}), language: settings.language });
      await initI18n();
      step = 2;
    } else if (step === 2) {
      const result = await stepOutputDir();
      if (result === '__BACK__') { step = 1; continue; }
      settings.outputDir = result;
      step = 3;
    } else if (step === 3) {
      const result = await stepTheme();
      if (result === '__BACK__') { step = 2; continue; }
      settings.theme = result;
      step = 4;
    } else if (step === 4) {
      const result = await stepConfirm(settings);
      if (result === '__BACK__') { step = 3; continue; }
      if (result === 'save') {
        // Save all settings
        const config = loadConfig(CONFIG_NAME, {});
  ...
  const { theme } = await inquirer.prompt([
    {
      ...
      choices: [
        { name: `${t('first_run.theme_default')}  [\x1b[34m████\x1b[0m]`, value: 'default' },
        { name: `${t('first_run.theme_minimal')}  [    ]`, value: 'minimal' },
        { name: `${t('first_run.theme_vibrant')}  [\x1b[35m█\x1b[36m█\x1b[33m█\x1b[31m█\x1b[0m]`, value: 'vibrant' },
...
async function stepConfirm(settings) {
  const locales = getSupportedLocales();
  const langLabel = locales.find(l => l.code === settings.language)?.label ?? settings.language;
  const themeLabels = {
  ...
  console.log('  ├──────────────────────────────────────┤');
  console.log(`  │  ${t('first_run.confirm_language')}: ${langLabel}`.padEnd(40) + '│');
  console.log(`  │  ${t('first_run.confirm_output')}: ${settings.outputDir}`.padEnd(40) + '│');
  console.log(`  │  ${t('first_run.confirm_theme')}: ${themeLabels[settings.theme]}`.padEnd(40) + '│');
  console.log('  └──────────────────────────────────────┘');
  console.log('');
