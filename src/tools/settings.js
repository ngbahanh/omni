...
async function sectionLanguage() {
  ...
  const currentLabel =
    locales.find((l) => l.code === currentLang)?.label ?? currentLang;
...
async function sectionAuth() {
  ...
  console.log("");
  console.log(
    `  ${t("settings.auth_platform").padEnd(14)} ${t("settings.auth_status").padEnd(18)} ${t("settings.auth_action")}`,
  );
  ...
  for (const p of PLATFORMS) {
    ...
    console.log(
      `  ${p.label.padEnd(14)} ${statusText.padEnd(18)} ${actionText}`,
    );
  }
...
async function managePlatformAuth(platformKey, config) {
  ...
  if (action === "remove") {
    ...
    if (confirm) {
      ...
      saveConfig(CONFIG_NAME, config);
      ui.success(
        t("settings.auth_cookies_removed", { platform: platform.label }),
      );
    }
    return;
  }
  ...
  try {
    ...
    execSync(
      `yt-dlp --cookies "${resolvedPath}" --simulate "${platform.testUrl}"`,
      {
        timeout: 30000,
        stdio: "pipe",
      },
  ...
  if (testOk) {
    ...
    if (saveAnyway) {
      ...
      saveConfig(CONFIG_NAME, config);
      ui.success(
        t("settings.auth_cookies_saved", { platform: platform.label }),
      );
    }
  }
}
    ...
    const { option } = await inquirer.prompt([
      {
        ...
        choices: [
          {
            name: `${t("settings.advanced_sleep")}  [${config.sleepInterval[0]}, ${config.sleepInterval[1]}]s`,
            value: "sleep",
          },
          {
            name: `${t("settings.advanced_retries")}  ${config.maxRetries}`,
            value: "retries",
          },
      ...
      const { min } = await inquirer.prompt([
        {
          type: "input",
          name: "min",
          message: t("settings.advanced_sleep_min"),
          default: String(config.sleepInterval[0]),
        },
      ]);
      const { max } = await inquirer.prompt([
        {
          type: "input",
          name: "max",
          message: t("settings.advanced_sleep_max"),
          default: String(config.sleepInterval[1]),
        },
      ]);
      ...
      const { retries } = await inquirer.prompt([
        {
          type: "input",
          name: "retries",
          message: t("settings.advanced_retries_prompt"),
          default: String(config.maxRetries),
        },
      ]);
      ...
      const { template } = await inquirer.prompt([
        {
          ...
          choices: [
            {
              name: "%(upload_date)s - %(title)s [%(id)s].%(ext)s",
              value: "%(upload_date)s - %(title)s [%(id)s].%(ext)s",
            },
            {
            ...
            {
              name: `✏ ${t("settings.advanced_template_custom")}`,
              value: "__CUSTOM__",
            },
          ],
        },
      ...
      const { confirm } = await inquirer.prompt([
        {
          type: "confirm",
          name: "confirm",
          message: t("settings.advanced_reset_confirm"),
          default: false,
        },
      ]);
      if (confirm) {
...
async function sectionAbout() {
  ...
  console.log("  ┌────────────────────────────────────────────┐");
  console.log(
    `  │  ${t("settings.about_version", { version: pkg.version }).padEnd(42)}│`,
  );
  console.log(
    `  │  ${t("settings.about_node", { version: process.version }).padEnd(42)}│`,
  );
  console.log("  ├────────────────────────────────────────────┤");
  ...
  for (const dep of deps) {
    try {
      const output = execSync(dep.cmd, { timeout: 5000, stdio: "pipe" })
        .toString()
        .trim();
      const version =
        dep.name === "ffmpeg"
          ...
          : output.split("\n")[0].trim();
      console.log(
        `  │  ${t("settings.about_dep_ok", { name: dep.name, version }).padEnd(42)}│`,
      );
    } catch {
      console.log(
        `  │  ${t("settings.about_dep_missing", { name: dep.name }).padEnd(42)}│`,
      );
    }
  }
  ...
  console.log("  ├────────────────────────────────────────────┤");
  console.log(
    `  │  ${t("settings.about_config_path")}: .agents/config/omni-global.json`.padEnd(
      44,
    ) + "│",
  );
  console.log(
    `  │  ${t("settings.about_logs_path")}: .agents/logs/omni.log`.padEnd(44) +
      "│",
  );
  console.log("  └────────────────────────────────────────────┘");
  console.log("");
