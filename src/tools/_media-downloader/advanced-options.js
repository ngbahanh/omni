    ...
    const { count } = await inquirer.prompt([
      {
        ...
        validate: (input) => {
          ...
          if (isNaN(n) || n <= 0) return t('downloader.advanced_limit_positive');
          return true;
        },
      },
    ]);
    ...
    const { preset } = await inquirer.prompt([
      {
        ...
        choices: [
          { name: t('downloader.advanced_template_default'), value: '%(title)s [%(id)s].%(ext)s' },
          { name: t('downloader.advanced_template_date'), value: '%(upload_date)s - %(title)s.%(ext)s' },
          { name: t('downloader.advanced_template_index'), value: '%(playlist_index)s - %(title)s.%(ext)s' },
          { name: t('downloader.advanced_template_enter'), value: '__custom__' },
        ],
      },
    ]);
